...
$(document).ready(function (){
   ...
   dropDownCountryStates = $("#dropDownCountriesForStates");
   ...
   dropDownCountryStates.on("change",function (){
      loadStatesCountry();
   });

   dropDownStates.on("change",function (){
      changeFormStateTpSelectedState();
   });
...
function deleteState(){
   ...
   url = "http://localhost:8080/ShopmeAdmin/states/delete/" + stateId;
...
function updateState(){
   ...
   url = "http://localhost:8080/ShopmeAdmin/states/save";
   stateId = dropDownStates.val();
...
function changeFormStateTpSelectedState(){
...
function loadStatesCountry(){
   selectedCountry = $("#dropDownCountriesForStates option:selected");
   countryId = selectedCountry.val();
   url = "http://localhost:8080/ShopmeAdmin/states/list_by_country/" + countryId;
...
function loadCountriesStates(){
   url = "http://localhost:8080/ShopmeAdmin/countries/list";
   $.get(url,function (responseJSON){
      dropDownCountryStates.empty();

      $.each(responseJSON,function (index,country){
         $("<option>").val(country.id).text(country.name).appendTo(dropDownCountryStates);
      });
   }).done(function (){
